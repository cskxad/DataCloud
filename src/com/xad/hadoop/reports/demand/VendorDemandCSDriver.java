 ...
 *
 */
public class VendorDemandCSDriver {

    public static void main(String[] args) throws Exception {
        Configuration conf = new Configuration();
        String[] otherArgs = new GenericOptionsParser(conf, args).getRemainingArgs();
        if (otherArgs.length < 2) {
            System.err.println("Usage: VendorDemandCSDriver <in> <out> [jars]");
            System.exit(2);
        }

        Job job = new Job(conf, "VendorDemandCSDriver");

        for (int i = 2; i < otherArgs.length; i++) {
            ((JobConf)job.getConfiguration()).setJar(args[i]);
        }

        job.setMapperClass(VendorDemandCityStateMapper.class);
        job.setNumReduceTasks(12);
        job.setReducerClass(VendorDemandCityStateReducer.class);
