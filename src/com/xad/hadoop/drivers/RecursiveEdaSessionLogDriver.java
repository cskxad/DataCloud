 ...
 *
 */
public class RecursiveEdaSessionLogDriver {

    public static void main(String[] args) throws Exception {
        if (args.length < 2) {
            System.err.println("Usage: com.xad.hadoop.drivers.SearchDataDriver <input path> <output path> startDate counter [jars]");
            System.exit(-1);
        }
        ...
        int maxCount = Integer.parseInt(otherArgs[3]);
        System.out.println("Mac count = "+maxCount);

        int startDate = Integer.parseInt(otherArgs[2]);

        String inputPathPrefix = "input/";
        String inputPathSuffix = "/search/edasessionlog";
        
        int counter = 0;
        while(counter < maxCount) {

            Job job = new Job(conf, "EdaSessionLog - "+startDate);
