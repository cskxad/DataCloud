 ...
 *
 */
public class VendorFillRateDriver {

    public static void main(String[] args) throws Exception {
        if (args.length < 2) {
            System.err.println("Usage: VendorFillRateDriver <input path> <output path> [jars]");
            System.exit(-1);
        }
        ...
        if (otherArgs.length < 2) {
            System.err.println("Usage: VendorFillRateDriver <in> <out> [jars]");
            System.exit(2);
        }

        Job job = new Job(conf, "VendorFillRateDriver");

        for (int i = 2; i < otherArgs.length; i++) {
            ((JobConf)job.getConfiguration()).setJar(args[i]);
        }

        job.setMapperClass(VendorFillRateMapper.class);
        job.setNumReduceTasks(6);
        job.setReducerClass(VendorFillRateReducer.class);
